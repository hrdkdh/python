...
def splitGroup(cha_name):
    ...
    for ban in ban_arr:
        ...
        #총 조 갯수 계산
        total_group_cnt = round(len(this_df)/group_member_number_limit)

        #우선순위대로 정렬
        order_cit_arr = ["대학명", "계열", "성별", "거주지_시", "거주지_도"]
        this_df.sort_values(by=order_cit_arr, inplace=True)
        this_df.reset_index(drop=True, inplace=True) #인덱스 리셋
        ...
        for i in range(group_member_number_limit):
            ...
            for idx in range(this_start_num, this_end_num):
                group_number += 1
                this_df._set_value(idx, "조", group_number)
        
        order_cit_arr2 = ["조", "성명"]
        this_df.sort_values(by=order_cit_arr2, inplace=True)
        this_df.reset_index(drop=True, inplace=True) #인덱스 리셋
        ...
        for idx in this_df.index:
            ...
            if print_no > group_member_number_limit:
                print_no = 1
            this_df._set_value(idx, "출력순서", print_no)
        df_arrays.append(this_df)

    df_results = pd.concat(df_arrays, ignore_index=True)
    makeDownloadDirectory()

    df_results.to_excel(filesave_path+"조편성표_"+cha_name+".xlsx", sheet_name="조편성 명단")
    print("조편성 완료")
